...
pub struct Phy<'a, const INST: u8, const MTU: usize, const RX_LEN: usize, const TX_LEN: usize>
{
    tx_pos: usize,
    rx_pos: usize,
...
impl<const INST: u8, const MTU: usize, const RX_LEN: usize, const TX_LEN: usize> phy::Device
    for Phy<'_, INST, MTU, RX_LEN, TX_LEN>
{
    //these statements, and the associated borrow logic, specifies that the token has the lifetime of the Phy object. This means there can only ever be each of the TX and RX Tokens.
    type RxToken<'a> = PhyRxToken<'a, MTU, RX_LEN> where Self: 'a;
    type TxToken<'a> = PhyTxToken<'a, INST, MTU, TX_LEN> where Self: 'a;
    ...
    fn transmit(&mut self, _timestamp: Instant) -> Option<Self::TxToken<'_>> {
        ...
        if (desc.flags & 0x8000) == 0x0 {
            let tok: PhyTxToken<'_, INST, MTU, TX_LEN> = PhyTxToken {
                tx_pos: &mut self.tx_pos,
                enet_inst: &mut self.enet_inst,
                txdt: &mut self.txdt,
            };
...
pub struct PhyRxToken<'a, const MTU: usize, const RX_LEN: usize> {
    rx_pos: &'a mut usize,
    rxdt: &'a mut RxDT<MTU, RX_LEN>,
}

impl<'a, const MTU: usize, const RX_LEN: usize> phy::RxToken for PhyRxToken<'a, MTU, RX_LEN> {
...
pub struct PhyTxToken<'a, const INST: u8, const MTU: usize, const TX_LEN: usize> {
    tx_pos: &'a mut usize,
...
impl<'a, const INST: u8, const MTU: usize, const TX_LEN: usize> phy::TxToken
    for PhyTxToken<'a, INST, MTU, TX_LEN>
{
